...
export class CohereEmbeddingService {
    ...
    async generateBatchEmbeddings(texts, inputType = 'search_document') {
        ...
        return this.executeWithRetry(async () => {
            ...
            try {
                ...
                const embeddings = response.embeddings.float.map((embedding) => {
                    if (!embedding || embedding.length === 0) {
     ...
     */
    static findMostSimilar(queryEmbedding, candidateEmbeddings, threshold = 0.5) {
...
export function getCohereService() {
    if (!cohereService) {
