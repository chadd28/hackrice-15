...
export class EmbeddingCache {
    ...
    async storeBatchEmbeddings(embeddings) {
        const validEmbeddings = embeddings.filter(item => item.id && item.text && item.embedding && item.embedding.length > 0);
    ...
    async persistCache() {
        try {
            ...
            const questionIds = Array.from(new Set(cacheData
                .map(embedding => embedding.questionId)
                .filter((id) => id !== undefined))).sort((a, b) => a - b);
            const metadata = {
     ...
     */
    async storeQuestionEmbedding(questionId, text, embedding) {
        if (!this.isLoaded) {
     ...
     */
    async getQuestionEmbedding(questionId) {
        if (!this.isLoaded) {
            await this.initialize();
        }
        const id = `question_${questionId}`;
     ...
     */
    async hasQuestionEmbedding(questionId) {
        if (!this.isLoaded) {
            await this.initialize();
        }
        const id = `question_${questionId}`;
     ...
     */
    async storeQuestionEmbeddings(embeddings) {
        if (!this.isLoaded) {
            await this.initialize();
        }
