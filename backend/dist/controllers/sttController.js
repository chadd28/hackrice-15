...
export const transcribeAudio = async (req, res) => {
    try {
        ...
        const longRunningResponse = await axios.post(`https://speech.googleapis.com/v1/speech:longrunningrecognize?key=${API_KEY}`, requestBody, {
        ...
        while (!operationComplete && attempts < maxAttempts) {
            ...
            try {
                const statusResponse = await axios.get(`https://speech.googleapis.com/v1/operations/${operationName}?key=${API_KEY}`);
        ...
        if (data && data.results && data.results.length > 0) {
            ...
            const confidences = data.results
                .filter((result) => result.alternatives && result.alternatives[0].confidence !== undefined)
...
export const transcribeChunk = async (req, res) => {
    try {
        const API_KEY = process.env.GOOGLE_STT_API_KEY;
        if (!API_KEY) {
            throw new Error("Google Speech-to-Text API key not found in environment variables");
        ...
        const response = await axios.post(`https://speech.googleapis.com/v1/speech:recognize?key=${API_KEY}`, requestBody, {
            headers: { 'Content-Type': 'application/json' }
        });
