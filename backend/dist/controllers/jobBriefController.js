...
/*  Ranking logic                                                     */
/* ------------------------------------------------------------------ */
function pickBestPosting(results, company, title) {
    const lc = (s) => s?.toLowerCase() ?? "";
    const c = lc(company);
    const t = lc(title);
    // 1) ATS-hosted postings that match title
    const atsHits = results.filter((r) => r.url &&
    ...
    const careerHit = results.find((r) => r.url &&
        lc(r.url).includes(c) &&
...
/*  Controller                                                        */
/* ------------------------------------------------------------------ */
export async function jobBrief(req, res) {
    try {
        ...
        const query = `${company} "${title}" (Workday OR Greenhouse OR Lever OR ` +
            `site:careers.${companySlug}.com OR site:jobs.${companySlug}.com)`;
        const results = await tavilySearch(query, 8);
        if (results.length === 0)
            return res.json({ notFound: true, sources: [] });
        const best = pickBestPosting(results, company, title) ?? results[0];
        if (!best?.url)
            return res.json({ notFound: true, sources: [] });
        const rawContent = best.content?.trim().length ? best.content : await tavilyExtract(best.url);
