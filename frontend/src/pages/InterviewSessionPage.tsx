...
import { ArrowLeft, ArrowRight, RotateCcw, CheckCircle, Clock, Mic, MicOff } from 'lucide-react';
...
const InterviewSessionPage: React.FC = () => {
    ...
    const loadQuestions = async () => {
      try {
        ...
        console.log('📊 Total Questions:', {
          technical: loadedQuestions.technical.length,
          behavioral: loadedQuestions.behavioral.length,
          total: loadedQuestions.technical.length + loadedQuestions.behavioral.length
        });
        
        console.group('🔧 Technical Questions');
        loadedQuestions.technical.forEach((q, index) => {
        ...
        loadedQuestions.behavioral.forEach((q, index) => {
          console.log(`${index + 1}. [${q.difficulty.toUpperCase()}] ${q.question}`);
          console.log(`   Tags: ${q.tags.join(', ')}`);
          console.log(`   Context: ${q.context}`);
          console.log('---');
        ...
        if (loadedQuestions.technical.length > 0) {
          setCurrentQuestion(loadedQuestions.technical[0]);
          setCurrentCategory('technical');
          setCurrentQuestionIndex(0);
        } else if (loadedQuestions.behavioral.length > 0) {
          setCurrentQuestion(loadedQuestions.behavioral[0]);
  ...
  const goToPreviousQuestion = () => {
    if (currentQuestionIndex > 0) {
      ...
      setCurrentCategory('technical');
      const lastTechnicalIndex = questions.technical.length - 1;
      setCurrentQuestionIndex(lastTechnicalIndex);
      setCurrentQuestion(questions.technical[lastTechnicalIndex]);
      
      console.log('🔄 Switching back to technical questions');
      console.log('📍 Last technical question:', questions.technical[lastTechnicalIndex].question);
    }
  };
  ...
  /**
   * Toggle recording state (simulation)
   */
  const toggleRecording = () => {
    setIsRecording(!isRecording);
    console.log(isRecording ? '⏹️ Stopped recording' : '🎤 Started recording');
  };
